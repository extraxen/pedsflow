...
class _SodiumReplacementScreenState extends State<SodiumReplacementScreen>{final weight=TextEditingController(text:'20'),current=TextEditingController(text:'126'),target=TextEditingController(text:'132');double tbw=.6;@override void dispose(){weight.dispose();current.dispose();target.dispose();super.dispose();}double? n(c)=>double.tryParse(c.text.trim());@override Widget build(BuildContext context){final W=n(weight),C=n(current),T=n(target);final deficit=W!=null&&C!=null&&T!=null?(T-C)*W*tbw:null;final delta=C!=null&&T!=null?T-C:null;final over=delta!=null&&delta>8;return _ePage(context,'Sodium correction safety',[
 _eNotice(context,'For a non-seizing child with hyponatremia, RCH targets correction of about 6â€“8 mmol/L over 24 h. Neurologic symptoms/seizures require an emergency hypertonic-saline protocol and senior/PICU input; this screen does not substitute for that order set.'),
 ...
 if(over)const Card(color:Color(0xFFFFE4E4),child:Padding(padding:EdgeInsets.all(12),child:Text('âš  Planned correction exceeds 8 mmol/L in 24 h. Re-enter a safer target unless a specialist emergency indication requires a different strategy.'))),
...
class _MagnesiumReplacementScreenState extends State<MagnesiumReplacementScreen>{final weight=TextEditingController(text:'20');double oral=.2,iv=.2,hours=3;bool renal=false;@override void dispose(){weight.dispose();super.dispose();}double? get w=>double.tryParse(weight.text.trim());@override Widget build(BuildContext context){final W=w;final oralDose=W==null?null:W*oral;final ivDose=W==null?null:math.min(W*iv,8);final rate=ivDose==null?null:ivDose/hours;return _ePage(context,'Hypomagnesemia replacement',[
 ...
 _eField(weight,'Weight','kg',()=>setState((){})),SwitchListTile(contentPadding:EdgeInsets.zero,title:const Text('Moderateâ€“severe renal impairment'),value:renal,onChanged:(v)=>setState(()=>renal=v)),
 const Text('Oral dose'),Slider(value:oral,min:.1,max:.8,divisions:7,label:'${oral.toStringAsFixed(1)} mmol/kg/dose',onChanged:(v)=>setState(()=>oral=v)),if(oralDose!=null)_eResult('Oral/enteral amount','${oralDose.toStringAsFixed(1)} mmol/dose â€¢ RCH usual 0.1â€“0.2 mmol/kg TID; higher 0.4â€“0.8 mmol/kg up to QID if required/tolerated'),
 ...
 if(renal)const Card(color:Color(0xFFFFE4E4),child:Padding(padding:EdgeInsets.all(12),child:Text('âš  Renal impairment increases risk of hypermagnesemia. Seek renal/pharmacy/specialist advice before replacement.'))),
 const Text('Monitor Mg according to severity and symptoms. IV magnesium 0.1â€“0.2 mmol/kg, up to 0.4 mmol/kg, max 8 mmol, is generally administered over 2â€“4 h in the RCH guideline; faster administration is reserved for severe symptoms with expert monitoring.'),
...
class _PhosphateReplacementScreenState extends State<PhosphateReplacementScreen>{final weight=TextEditingController(text:'20');double iv=.36;double hours=6;bool neonate=false,renal=false,central=false;@override void dispose(){weight.dispose();super.dispose();}double? get w=>double.tryParse(weight.text.trim());@override Widget build(BuildContext context){final W=w;final dose=W==null?null:W*iv;final rate=dose==null?null:dose/hours;final double maxConc = central ? 0.12 : 0.05;final minVol=dose==null?null:dose/maxConc;final maxRateW=W==null?null:.2*W;return _ePage(context,'Hypophosphatemia replacement',[
 ...
 _eResult('Enteral reference',neonate?'1 mmol/kg/day in 2â€“4 doses; up to 3 mmol/kg/day may be used for osteopenia of prematurity under neonatal guidance':'2â€“3 mmol/kg/day in 2â€“4 divided doses for children (product-specific maximums apply)'),
 ...
 if(dose!=null)_eResult('IV phosphate amount','${dose.toStringAsFixed(2)} mmol'),if(rate!=null)_eResult('Infusion rate','${rate.toStringAsFixed(2)} mmol/hr${maxRateW==null?'':' â€¢ weight-based ceiling ${maxRateW.toStringAsFixed(1)} mmol/hr'}'),if(minVol!=null)_eResult('Minimum final volume at RCH concentration ceiling','${minVol.toStringAsFixed(0)} mL at â‰¤${maxConc.toStringAsFixed(2)} mmol/mL (${central?'central':'peripheral'})'),
 if(dose!=null&&dose>10)const Card(color:Color(0xFFFFF0D2),child:Padding(padding:EdgeInsets.all(12),child:Text('âš  IV phosphate dose >10 mmol: RCH recommends senior clinician discussion.'))),if(renal)const Card(color:Color(0xFFFFE4E4),child:Padding(padding:EdgeInsets.all(12),child:Text('âš  Reduce dose and obtain specialist advice in renal impairment.'))),
 const Text('RCH IV reference: 0.36 mmol/kg slow infusion; peripheral concentration â‰¤0.05 mmol/mL, central â‰¤0.12 mmol/mL; usually over 6 h; maximum rate 0.2 mmol/kg/h or 10 mmol/h. Check phosphate and potassium before and ~2 h after completion. Do not share a lumen with calcium- or magnesium-containing solutions.'),
