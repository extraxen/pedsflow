...
class SearchHit {
  ...
  const SearchHit(this.document, this.score, this.matchedOn);
}
...
class GlobalSearchIndex {
  ...
  static const Map<String, String> _aliases = <String, String>{
    'vomitting': 'vomiting', 'emesis': 'vomiting', 'gravol': 'dimenhydrinate',
    'zofran': 'ondansetron', 'hypok': 'hypokalemia', 'hyperk': 'hyperkalemia',
    'hypophos': 'hypophosphatemia', 'bronch': 'bronchiolitis', 'pna': 'pneumonia',
    'icp': 'raised intracranial pressure', 'chs': 'cannabis hyperemesis syndrome',
    'tylenol': 'acetaminophen', 'paracetamol': 'acetaminophen', 'motrin': 'ibuprofen',
    'advil': 'ibuprofen', 'dilaudid': 'hydromorphone', 'epi': 'epinephrine',
    'norad': 'norepinephrine', 'noradrenaline': 'norepinephrine',
    'status asthma': 'status asthmaticus', 'rsi': 'rapid sequence intubation',
    'ett': 'intubation', 'lp': 'lumbar puncture', 'tbi': 'traumatic brain injury',
    'roc': 'rocuronium', 'roconium': 'rocuronium', 'nimbex': 'cisatracurium',
    'precedex': 'dexmedetomidine', 'dex': 'dexmedetomidine', 'versed': 'midazolam',
    'fent': 'fentanyl', 'prop': 'propofol', 'sux': 'succinylcholine',
    'clavulin': 'amoxicillin clavulanate',
    'zosyn': 'piperacillin tazobactam', 'pip tazo': 'piperacillin tazobactam',
    'keppra': 'levetiracetam', 'lasix': 'furosemide', 'solumedrol': 'methylprednisolone',
    'solu medrol': 'methylprednisolone', 'bactrim': 'trimethoprim sulfamethoxazole',
    'septra': 'trimethoprim sulfamethoxazole', 'rocephin': 'ceftriaxone',
    'ancef': 'cefazolin', 'flagyl': 'metronidazole', 'diflucan': 'fluconazole',
    'ampho b': 'amphotericin b',
    'suxamethonium': 'succinylcholine', 'dka cerebral edema': 'cerebral edema in dka',
    'sickle crisis': 'vaso occlusive pain', 'voc': 'vaso occlusive pain',
  ...
  static Future<GlobalSearchIndex> build(AppStore store) async {
    ...
    final String raw = await rootBundle.loadString('assets/static_search_index.json');
    docs.addAll((jsonDecode(raw) as List<dynamic>)
        .map((dynamic item) => SearchDocument.fromJson(item as Map<String, dynamic>)));
    ...
    for (final AdmissionPlan plan in store.plans) {
      docs.add(SearchDocument(
        ...
        body: <String>[
          ...
          ...plan.sourceLabels,
          ...plan.sections.asMap().entries.expand((entry) => <String>[entry.value.label, store.sectionText(plan, entry.key)]),
          ...plan.treatments.asMap().entries.expand((entry) => <String>[entry.value.group, store.treatmentText(plan, entry.key)]),
        ].join(' '),
      ));
    }
    ...
    for (final MedicationMonograph med in store.medications) {
      docs.add(SearchDocument(
        ...
        body: <String>[
          ...
          med.verificationStatus, med.sourceSummary, med.currentCanadianMonograph,
          ...med.sourceLabels,
          ...med.linkedPlans.expand((p) => <String>[p.planTitle, p.category, p.excerpt]),
          ...med.doseSections.expand((d) => <String>[d.title, d.routeGroup, d.text, d.source, d.sourceDate]),
        ].join(' '),
      ));
    }
    ...
    if (guide != null) {
      ...
      for (final CoveragePearl p in guide.coveragePearls) {
        docs.add(SearchDocument(
          id: 'abxpearl:${p.title}', title: p.title, category: 'Antibiotic coverage pearl', kind: 'antibiotic',
          target: p.title, aliases: const [], body: p.text,
        ));
      }
    }
  ...
  List<SearchHit> search(String rawQuery, {String? kind, int limit = 80}) {
    ...
    for (final SearchDocument doc in documents) {
      ...
      if (title == normalized) { score += 1500; matchedOn = 'title'; }
      else if (title.startsWith(normalized)) { score += 1100; matchedOn = 'title'; }
      else if (title.contains(normalized)) { score += 850; matchedOn = 'title'; }
      if (aliases.contains(normalized)) { score += 950; matchedOn = 'alias'; }
      if (category.contains(normalized)) score += 250;
      if (body.contains(normalized)) score += 150;

      for (final String token in tokens) {
        if (title.split(' ').contains(token)) score += 220;
        else if (title.split(' ').any((w) => w.startsWith(token))) score += 160;
        if (aliases.split(' ').contains(token)) score += 180;
        if (category.split(' ').contains(token)) score += 60;
        if (body.contains(token)) score += 25;
      }
      if (doc.kind == 'pccu' || doc.kind == 'medication' || doc.kind == 'plan') score += 20;
      hits.add(SearchHit(doc, score, matchedOn));
    }

    hits.sort((a, b) {
      final int byScore = b.score.compareTo(a.score);
      return byScore != 0 ? byScore : a.document.title.compareTo(b.document.title);
    });
  ...
  static bool _allTokensMatch(List<String> tokens, String haystack) {
    ...
    for (final String token in tokens) {
      ...
      final bool fuzzy = words.any((word) => _fuzzyToken(token, word));
  ...
  static int _levenshtein(String a, String b, int stopAt) {
    ...
    for (int i = 1; i <= a.length; i++) {
      ...
      for (int j = 1; j <= b.length; j++) {
        ...
        current[j] = _min3(current[j - 1] + 1, previous[j] + 1, previous[j - 1] + cost);
