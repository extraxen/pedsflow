  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: <Widget>[
        const _RequiredInputsCard(),
        const SizedBox(height: 12),
        const _InfoSection(title: 'Spectrum symbols', body: 'â— expected activity   â— variable/context-dependent   â—‹ generally unreliable\nG+ Gram-positive | MRSA | G- Gram-negative | PsA Pseudomonas | AN anaerobes | AT atypicals | F fungal'),
        const SizedBox(height: 12),
...
const Map<String, String> _antibioticSyndromes = {
  'Sepsis / meningitis': 'Targets: G+, G-, age-specific organisms; MRSA and PsA only when risk justifies. Framework: broad antipseudomonal beta-lactam Â± MRSA agent for septic shock; third-generation cephalosporin + vancomycin for bacterial meningitis with age-specific additions. Obtain cultures when this does not delay therapy. Confirm meningitis dosing, renal adjustment, HSV criteria and source control.',
  'Pneumonia': 'Uncomplicated CAP: narrow pneumococcal-active beta-lactam when appropriate. Severe/post-influenza: consider S. aureus/MRSA based on features. Add atypical coverage selectively. Evaluate effusion/empyema and drainage. Spectrum: G+ â— | MRSA â—‹/risk-based | G- â— | PsA â—‹ unless healthcare-associated | AN â—‹ unless aspiration syndrome | AT agent-dependent.',
  ...
  'Skin / soft tissue': 'Nonpurulent cellulitis: streptococci Â± MSSA. Purulent abscess: drainage first; MRSA therapy by severity/local prevalence. Necrotizing infection: urgent surgery plus broad therapy and toxin suppression.',
  'Bone / joint': 'Target S. aureus, streptococci and age-specific organisms. Obtain blood/joint cultures; involve orthopedics for aspiration/drainage; choose MSSA-active therapy Â± MRSA based on risk and local susceptibility.',
...
const Map<String, String> _emergencyProtocols = {
  ...
  'Vasoactive medication selection': 'Assess BP/perfusion, fluid already given, cardiac function, warm/cold phenotype, lactate, access, arrhythmia and pulmonary hypertension.\nâ€¢ Persistent shock after carefully reassessed fluid: start vasoactive support without waiting for central access when local policy permits.\nâ€¢ Low SVR/warm shock: vasoconstrictor-dominant pathway.\nâ€¢ Low output/cold shock: inotrope/vasoactive selection based on BP and echo.\nâ€¢ Cardiogenic shock: avoid repeated empiric boluses.\nOutput must include agent, local weight-based starting rate, access, titration target, continuous ECG/BP and reassessment q5â€“15 min.',
  'Status epilepticus': '0â€“5 min: ABCs, oxygen/suction, glucose, IV/IO, record onset, first-line benzodiazepine per local weight-based protocol.\n5â€“10 min: confirm adequate dose, repeat only per pathway, prepare ventilation, correct glucose.\n10â€“20 min: second-line antiseizure agent selected by contraindications and formulary.\nPersistent: refractory pathway, PICU/anesthesia, intubation/anesthetic therapy, continuous EEG and etiologic evaluation.\nPitfall: repeated benzodiazepines without cumulative-dose and ventilation review.',
  'DKA': 'Confirm hyperglycemia + ketosis + metabolic acidosis. Assess perfusion, neurologic status, K, sodium trend, renal function and prior fluids. Restore perfusion if needed; calculate maintenance + deficit âˆ’ prior fluids using local pediatric protocol. Start continuous insulin after initial fluid and K review; no routine bolus. Add K only with renal/urine-output assessment. Increase dextrose as glucose falls so insulin continues until ketoacidosis resolves. Hourly glucose/neuro checks; labs q2â€“4h. Treat cerebral injury immediately without delaying for imaging.',
  'Sepsis / septic shock': 'Recognize suspected infection with organ dysfunction/shock. ABCs, lactate, IV/IO, cultures if no meaningful delay, prompt broad empiric antibiotics and source search. Give reassessed 10â€“20 mL/kg fluid aliquots only when indicated; stop for improvement or fluid overload. Start vasoactive support for fluid-refractory shock. Trend perfusion/lactate, narrow therapy and pursue source control.',
